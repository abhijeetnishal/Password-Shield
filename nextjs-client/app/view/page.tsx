...
const PasswordPage = () => {
    ...
    const [data, setData] = useState<{websitename?: string, websiteName?: string, _id: string }[]|null>(null);
    const [decryptedPassword, setDecryptedPassword] = useState<{decryptedPassword: string, id: string}|null>(null);
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            const response = await fetch(`${process.env.REACT_APP_HOST_URL}/passwords/all/${cookieValue._id}`, {
                method: 'GET',
                headers: {
    ...
    return (
        <ProtectedRoute>
        <div data-testid="password-page" className='' style={{ height: `${containerHeight}px` }}>
            <div className=' flex flex-row justify-between items-center px-2 py-4'>
                <h1 className='font-medium'>
                    Your Saved Passwords
                </h1>
                <button className='text-white bg-teal-500 p-1 rounded' onClick={()=>handleAddClick()}>
            ...
            <div className=' m-2 p-2 lg:w-1/2'>
                ...
                (data !== null && dataLength && !isLoading) ? 
                    (data.map((mainData: {websitename?: string, websiteName?: string, _id: any }, index: number) => (
                        <div className='border rounded' key={index}>
                            <div className='flex flex-row items-center justify-start'>
                                <div>
                                <Image className='m-1' width={60} height={60} src={commonWebsiteSymbolImg} alt="" />
                                </div>
                                <div className='flex flex-col m-1 items-start  justify-center'>
                                    ...
                                    <button onClick={()=> decrypt(mainData._id)} className='ml-1'>
                                        {
                                            isPasswordEyeBtnClicked ? (<Image src={cutEye} alt=''  className=''/> )
                            ...
                            <div className='border-t flex flex-row items-center justify-end'>
                                <div className=''>
                                    <button className='flex border border-cyan-600 rounded p-1  flex-row items-center text-cyan-600' onClick={()=>handleEditClick(mainData._id)}>
                                ...
                                <div className=''>
                                    <button className='m-1 border rounded border-cyan-600  p-1 flex flex-row items-center text-cyan-600' onClick={()=>handleDeleteClick(mainData._id)}>
