...
const CreatePassword = (props: CreatePasswordProps) => {
  ...
  async function addFunc() {
    if (websiteName && password) {
      const response = await fetch(
        `${process.env.REACT_APP_HOST_URL}/passwords`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <div
      ...
      onClick={onClose}
      className=" min-h-[86vh] flex flex-row items-start justify-center bg-white p-5 "
    >
      <div
        onClick={(e) => {
          e.stopPropagation();
        }}
        className="shadow-md border p-6 rounded m-2 flex flex-col w-4/5 lg:w-1/3 "
      >
        <div className="">
        ...
        <p className="text-red-500 self-center m-1 font-light">{message}</p>
      </div>
    </div>
  );
};
